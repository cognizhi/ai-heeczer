...
BEGIN
    SELECT RAISE(ABORT, 'aih_events is append-only');
END;
CREATE TRIGGER aih_events_no_delete
BEFORE DELETE ON aih_events
BEGIN
    SELECT RAISE(ABORT, 'aih_events is append-only; use aih_tombstones');
END;
...
BEGIN
    SELECT RAISE(ABORT, 'aih_scores is append-only; re-score by inserting a new version');
END;
CREATE TRIGGER aih_scores_no_delete
BEFORE DELETE ON aih_scores
BEGIN
    SELECT RAISE(ABORT, 'aih_scores is append-only');
END;
