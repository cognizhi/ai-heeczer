...
│   │   ├── docker-compose.yml
│   │   ├── .env.example
│   │   └── chatbot/                # Node/TS chatbot app (Vite + Express)
│   │       ├── src/
│   │       │   ├── tools/
...
│   │   ├── docker-compose.yml
│   │   ├── .env.example
│   │   └── chatbot/                # Python chatbot app (FastAPI + htmx)
│   │       ├── tools/catalogue.py
│   │       ├── skills/catalogue.py
│   │       └── ...
│   ├── pydanticai/
...
- One named volume per stack: `heeczer-test-<sdk>-pgdata`.
- `docker compose down` keeps it. `down -v` (used by `make reset-test-<sdk>`) drops it.
- Postgres creds: per-stack random password generated by `make start-test-<sdk>`
  on first run and written into `.env` if missing; never committed.
- Migrations run automatically by `heeczer-ingest` boot
  (`HEECZER_DATABASE_URL=postgres://...`) — no separate migrate step.
...
- No target in this plan is added to the default `make test` gate. Stacks are
  opt-in. A nightly CI job (plan 0012 follow-up) runs `smoke-test-stacks` against
  a matrix of stacks with mocked LLM providers.
...
- Pinned image digests in `_shared/*.yml` for supply-chain hygiene; renovated
  via the existing dependency workflow.
...
- [ ] Add `testing/` skeleton: `README.md`, `compose/_shared/{ingest,dashboard,postgres,ollama}.yml`, `compose/_bin/` helpers.
- [ ] Add `.gitignore` rules for `testing/compose/*/.env`.
- [ ] Add `LLM_PROVIDER=mock` mode contract to `testing/README.md` (pseudocode + expected reply).
- [ ] **Author all six skill fixture files** under `testing/tests/fixtures/skills/` (§6 step 1). Fixtures are schema-validated against `core/schema/event.v1.json` as part of the PR CI check.
- [ ] Add `testing/tests/smoke/conftest.py` with shared fixtures (`compose_project`, `wait_for_http`, `mock_llm`, `skill_fixture_loader`).
- [ ] Add Make targets `start/stop/reset/logs/ps/smoke-test-<sdk>` as no-ops that error with "stack not yet implemented" for each SDK.
- [ ] Add aggregates `start-test-all`, `stop-test-all`, `reset-test-all`, `smoke-test-stacks`.
- [ ] Update `make help` and root README "Local stacks" section.
- [ ] Update plan 0013 to point its `docker-compose.dev.yml` bullet at this plan.

### JS/TS stack (depends on plan 0005)

- [ ] Failing smoke tests `testing/tests/smoke/test_js_stack.py` — one test per skill (6 total).
- [ ] `testing/compose/js/chatbot/src/tools/catalogue.ts` — all eight tool schemas + stubs (§4.8).
- [ ] `testing/compose/js/chatbot/src/skills/catalogue.ts` — six skill definitions + mock driver (§4.9).
- [ ] `testing/compose/js/chatbot/` Node + TypeScript app using `@cognizhi/heeczer-sdk` (Vite UI + Express API).
- [ ] Chat UI exposes `/skill <key>` command and a tool-call trace panel showing per-turn `ToolTrace`.
- [ ] `testing/compose/js/docker-compose.yml` with port band 18000–18099.
- [ ] `testing/compose/js/.env.example` with all four `LLM_PROVIDER` modes documented.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-js`.
- [ ] Smoke test green locally for all six skills; document in `testing/README.md`.

### Python stack (depends on plan 0006)

- [ ] Failing smoke tests `test_py_stack.py` — one per skill (6 total).
- [ ] `testing/compose/py/chatbot/tools/catalogue.py` — all eight tool schemas + stubs (§4.8).
- [ ] `testing/compose/py/chatbot/skills/catalogue.py` — six skill definitions + mock driver (§4.9).
- [ ] `testing/compose/py/chatbot/` FastAPI + htmx app using `heeczer` Python SDK.
- [ ] Chat UI exposes `/skill <key>` command and tool-call trace panel.
- [ ] `docker-compose.yml` with port band 18100–18199.
- [ ] `.env.example`.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-py`.
...
- [ ] Chat UI exposes `/skill <key>` command and tool-call trace panel.
- [ ] `docker-compose.yml` with port band 18500–18599.
- [ ] `.env.example`.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-pydanticai`.
- [ ] Adapter-shape assertion in the smoke test (`meta.extensions["chatbot.skill"]` present and `task.category` matches skill definition).

### Go stack (depends on plan 0007)

- [ ] Failing smoke tests `test_go_stack.py` — one per skill (6 total).
- [ ] `testing/compose/go/chatbot/tools/catalogue.go` — all eight tool schemas + stubs (§4.8).
- [ ] `testing/compose/go/chatbot/skills/catalogue.go` — six skill definitions + mock driver (§4.9).
- [ ] `testing/compose/go/chatbot/` Go HTTP service using `heeczer-go`.
- [ ] Chat UI exposes `/skill <key>` command and tool-call trace panel.
- [ ] `docker-compose.yml` with port band 18200–18299.
- [ ] `.env.example`.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-go`.

### Java stack (depends on plan 0009)

- [ ] Failing smoke tests `test_java_stack.py` — one per skill (6 total).
- [ ] `testing/compose/java/chatbot/tools/Catalogue.java` — all eight tool schemas + stubs (§4.8).
- [ ] `testing/compose/java/chatbot/skills/SkillCatalogue.java` — six skill definitions + mock driver (§4.9).
- [ ] `testing/compose/java/chatbot/` Spring Boot app using `heeczer-java`.
- [ ] Chat UI exposes `/skill <key>` command and tool-call trace panel.
- [ ] `docker-compose.yml` with port band 18300–18399.
- [ ] `.env.example`.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-java`.

### Rust stack (depends on plan 0008)

- [ ] Failing smoke tests `test_rs_stack.py` — one per skill (6 total).
- [ ] `testing/compose/rs/chatbot/src/tools/catalogue.rs` — all eight tool schemas + stubs (§4.8).
- [ ] `testing/compose/rs/chatbot/src/skills/catalogue.rs` — six skill definitions + mock driver (§4.9).
- [ ] `testing/compose/rs/chatbot/` Axum app using `heeczer` Rust SDK in-process.
- [ ] Chat UI exposes `/skill <key>` command and tool-call trace panel.
- [ ] `docker-compose.yml` with port band 18400–18499.
- [ ] `.env.example`.
- [ ] Wire `start/stop/reset/logs/ps/smoke-test-rs`.
...
- [ ] `testing/README.md` — quickstart, port matrix, troubleshooting, per-stack table.
- [ ] Root README "Try a local stack" section linking here.
- [ ] Update `examples/README.md` with a "Want a full sandbox?" pointer.
- [ ] Tech Writer subagent review pass before marking the foundations PR done.
