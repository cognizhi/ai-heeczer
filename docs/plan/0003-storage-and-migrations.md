...
## Goal
Provide a portable storage layer with SQLite (local/dev) and PostgreSQL (production) backends sharing a single migration history, append-only `heec_*` event/score tables, and tenant-scoped queries.
...
- [x] Add `heec migrate up|status|verify` CLI subcommands (per ADR-0010; supersedes the prior separate migration-cli plan). (PR #1)
