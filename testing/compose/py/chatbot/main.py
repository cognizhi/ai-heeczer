...
from tools.catalogue import function_schemas, trace_for_tools

app = FastAPI(title="ai-heeczer Python local stack")
workspace_id = os.environ.get("CHATBOT_WORKSPACE_ID", "local-test-py")
sdk_label = os.environ.get("CHATBOT_SDK", "py")
...
async def call_provider(fixture: dict[str, Any], prompt: str, provider: str) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(
                ...
                json={
                    "model": model,
                    "messages": [
                        {"role": "system", "content": f"Run the {fixture['skill']} local-stack scenario without revealing raw prompts."},
    ...
    if provider == "local":
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            response.raise_for_status()
            payload = response.json()
        return {"prompt_tokens": None, "completion_tokens": None, "text": (payload.get("message") or {}).get("content") or "Local model completed."}
...
def build_event(fixture: dict[str, Any], usage: dict[str, Any], started: float) -> dict[str, Any]:
    ...
    context["tags"] = ["local-stack", sdk_label, fixture["skill"]]
    event = {
        "spec_version": "1.0",
        "event_id": str(uuid.uuid4()),
        "correlation_id": f"{sdk_label}-session:{int(time.time() * 1000)}",
        "timestamp": datetime.now(UTC).isoformat(),
        "framework_source": f"chatbot-{sdk_label}",
...
async def chat(request: ChatRequest) -> dict[str, Any]:
    ...
    submission = await heeczer_client.ingest_event(workspace_id=workspace_id, event=event)
    return {
        "ok": True,
        "skill": fixture["skill"],
